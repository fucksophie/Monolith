...
export class Database {
        ...
        setInterval(()=>{
            ...
            sessions.forEach(s => {
                if(s.expiryTime < Date.now()) {
                    ...
                    user.openSessions = user.openSessions.filter(z => z.sessionID !== s.sessionID);
    ...
    getAllSessions(): Session[] {
        return this.db.queryEntries
            <{ sessions: string }>("select * from users")
            .map(z => JSON.parse(z.sessions)).flat() as Session[];
    }

    getBySession(session: string): User|undefined {
        const rawUser = this.db.queryEntries
            <{ username: string, password: string, sessions: string }>("select * from users")
        .find(z => {
            return JSON.parse(z.sessions).find((y: Session) => y.sessionID == session)
        });
    ...
    setWithUsername(username: string, user: User) {
        if(!this.getByUsername(username)) {
            this.db.query("INSERT INTO users VALUES(?, ?, ?);", [
                user.username, user.password, JSON.stringify(user.openSessions)
            ])
        } else {
            this.db.query("UPDATE users SET username = ?, password = ?, sessions = ? WHERE username = ?", [
                user.username, user.password, JSON.stringify(user.openSessions), user.username
            ])
        }
    }
}
